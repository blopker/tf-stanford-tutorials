...
print '1e:'
print sess.run(tf.diag(tf.ones(6)))
...
sub = tf.subtract(x, y)
avg = tf.reduce_mean(sub)
mse = tf.sqrt(tf.reduce_mean(tf.square(sub)))
abs = tf.reduce_sum(tf.abs(sub))
cond = tf.cond(tf.less(avg, 0), lambda: mse, lambda: abs)
print sess.run(cond)
